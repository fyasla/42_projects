...
/*   Updated: 2021/08/29 00:36:39 by faysal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_arguments(int	arg_nb, char **arg_list)
{
	...
	i = 0;
	int_list = malloc (arg_nb * sizeof(int));
	if (!int_list)
		return (0);
...
t_stacks	*initiate_stacks(int arg_nb, char **arg_list)
{
	...
	i = arg_nb - 1;
	a_bottom = malloc(sizeof (t_list2 *));
	if (!a_bottom)
		return (NULL);
	b_bottom = malloc(sizeof (t_list2 *));
	if (!b_bottom)
		return (NULL);
	...
	*b_bottom = NULL;
	while (i >= 0)
	{
		ft_lst2addtop(stacks->a, ft_atoi(arg_list[i]));
		i--;
	}
	return (stacks);
}
